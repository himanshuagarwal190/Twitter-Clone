import axios from 'axios'
import React, {useState} from 'react'
import {Redirect} from 'react-router-dom'

const instance = axios.create({
    withCredentials: true
  })
  

export default function Signup(){
    const [email, setEmail] = useState('')
    const [password, setPassword] = useState('')
    const [handle, setHandle] = useState('')
    const [userCreateState, setUserCreateState] = useState(false)

    function onSubmit(e){
        e.preventDefault()
        instance.post('http://localhost:5000/api/signup',{
            email,
            password,
            handle
        })
        .then((response)=>{
            if(response.data.status === 200){
                alert(response.data.message)
                setUserCreateState(true)
            }
            else if(response.data.status === 401){
                alert(response.data.message)
            }
            else{
                console.log(response)
                alert('Error occured while sign up')
            }
        })
    }

    return(
        <div className="bg-gray-800 h-screen flex justify-center">
            {userCreateState? <Redirect to='/login' /> : null}
            <div className="m-10 flex flex-col">
                <img alt="twitter logo" className="h-10" src="data:image/svg+xml;base64," />
                <p className="m-5 text-2xl font-semibold text-center text-white">Sign up to Twitter</p>
                <form onSubmit={onSubmit} className="flex flex-col">
                    <input required onChange={e => setEmail(e.target.value)} className="w-96 mt-6 h-10 bg-gray-800 border border-white text-white rounded px-2" type="email" name="email" placeholder="Email"/>
                    <input required onChange={e => setPassword(e.target.value)} className="w-96 mt-6 h-10 bg-gray-800 border border-white text-white rounded px-2" type="password" name="password" placeholder="Password"/>
                    <input required onChange={e => setHandle(e.target.value)} className="w-96 mt-6 h-10 bg-gray-800 border border-white text-white rounded px-2" type="handle" name="handle" placeholder="@Handle"/>
                    <input className="w-96 mt-6 h-10 bg-gray-800 border border-white text-white rounded px-2 cursor-pointer" type="submit" value="Sign Up" />
                </form>
            </div>
        </div>
    )
}