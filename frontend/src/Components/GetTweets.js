...
export default function GetTweets() {
  ...
  useEffect(() => {
    instance.get("/api/tweet").then((response) => {
      if (response.data.status === 200) {
        const likeCount = {};
  ...
  return loading ? (
    ...
    getTweets.map((tweet) => {
      return (
        <div
          key={tweet._id}
          className="flex flex-col border border-white w-6/12 self-center mt-10"
        >
          ...
          <div className="flex justify-around h-10">
            ...
            >
              <i className="far fa-heart"></i> {likes[tweet._id]}
            </button>
            <button className="w-1/2 border text-white">
